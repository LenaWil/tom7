...
(Figure~\ref{cig:3and2}). Note that $\langle k_1, k_2, k_3 \rangle$ is
achievable for any $k_1$, $k_2 \in \{ 0, 1, 2 \}$, and some unknown
$k_3$ (projected out of this display). This is a significant
improvement over what was achievable in BMML with two players. It
suggests that with enough friends willing to follow a program, some
set of people are likely to be able to achieve any amount of drinks
between them (if they have the ability to construct the right rule
set!); see Section~\ref{sec:conjectures}.
...
avenues for generalization (and exploration), which are explored in
the next chapter.
...
\caption{All possible outcomes for the first two players in 3-player
  ...
  (TODO: Incorporate this into the text, perhaps after discussing
...
\begin{enumerate}
  ...
  \item Open: Can BM6L achieve all $\langle k_1, k_2 \rangle$? Seems
    unlikely, based on how many samples we need to get close to 100\%
    for BM7L.
  \item Open: Can BM7L achieve all $\langle k_1, k_2 \rangle$? Seems
    likely, given how close BM7L is for a single player. We know it
    can get 96.67\%, at least!
