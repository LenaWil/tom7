...
struct
  ...
  handle Items.Items s => (Hero.messagebox s;
                           raise Hero.Hero "couldn't load profile")
  (* | Profile.Profile s => (Hero.messagebox s;
                             raise Hero.Hero "couldn't load profile")*)
    ...
    let
      ...
      fun loopplay () =
          let
            ...
            fun noteoff (ch, note) = Sound.noteoff (ch, note)
          in
            List.app
            (fn (label, evt) =>
             (case label of
        ...
        case S.pollevent () of
          ...
          | SOME e =>
              (case Input.map e of
                 ...
                 | SOME _ => () (* XXX other buttons should do
                                   something funny? *)
                 | NONE =>
                   (case e of
                      ...
                      (* Might be able to reach configure menu
                         with unconfigured joystick.. *)
                      | E_JoyDown { button, which, ... } =>
                          select (Input.joy which)
                      | E_JoyHat { state, ... } =>
                  ...
                  ("^4" ^
                   (if percent = 99 orelse misses > 0 andalso misses <= 3
                    then "-" ^ Int.toString misses
                       ...
                       fn () =>
                       colorp percent
                       (if percent = 99 orelse misses > 0 andalso
                           misses <= 3
                        then ("-" ^ Int.toString misses)
                        else Int.toString percent ^ "%") ^ " " ^
              ...
              let in
                FontSmall.draw(screen, x, y,
                               (if sel then "^3" else "^2") ^
                                   profile_name);
                if sel
                then SmallFont.draw(screen,
                                    x + FontSmall.sizex_plain profile_name,
                                    (* share baseline *)
                                    y + (FontSmall.height -
                                         SmallFont.height - 3),
                                    " (^5edit^0)")
...
*)
      ...
      and signin () =
        let
          ...
          fun drawitem (CreateNew, x, y, sel) =
