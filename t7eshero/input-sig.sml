...
sig
  ...
  (* These are the kinds of events that the game can receive from
     configure devices. *)
  datatype inevent =
    StrumUp
  | StrumDown
  ...
  (* only for joysticks *)
  val setaxis : device ->
    { which : int, axis : axis, min : int, max : int } -> unit
  ...
  val save : unit -> unit
  (* Load after registering joysticks to restore saved configuration
     for them *)
  val load : unit -> unit
