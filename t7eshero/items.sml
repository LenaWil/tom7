...
struct
  ...
  fun byz ({ zindex = zindex1, ...} : item, { zindex = zindex2, ... } : item) =
    Int.compare (zindex1, zindex2)

  (* negative zindex, positive zindex. sorted by zindex. *)
  type worn = item list * item list

  fun award (l : item list) =
    List.find (fn { id, ... } => not (List.exists (fn { id = id', ... } : item => id = id') l)) (!items)
