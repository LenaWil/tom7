...
struct
    ...
    fun algorithm { guid : string,
                    ...
                    body : XML.tree list } : { url : string,
                                               title : string } option =
      (* Unnamed runs used to be filtered from the feed, but now
         something is automatically naming them based on where they happened.
         Running around my neighborhood probably doesn't warrant posting, so
         filter those out. *)
        if title = "Untitled" orelse title = "Pittsburgh Running"
        then NONE
        else
