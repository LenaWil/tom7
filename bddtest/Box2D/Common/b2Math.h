...
inline string sweeptos(const b2Sweep &sweep) {
  char buf[512];
  sprintf(buf, "lc: %.4f,%.4f c: %.4f,%.4f -- %.4f,%.4f "
          "@%.4f -- %.4f",
          sweep.localCenter.x, sweep.localCenter.y,
          sweep.c0.x, sweep.c0.y,
          sweep.c.x, sweep.c.y,
          sweep.a0, 
          sweep.a);
  return string(buf);
