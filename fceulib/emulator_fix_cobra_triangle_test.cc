...
static SerialResult RunGameSerially(const Game &game) {
  TRACE_SCOPED_ENABLE_IF(string::npos != game.cart.find("Triangle"));
...
# define CHECK_RAM_STEP(i) do {                                 \
  ...
  auto SaveAndStep = [&StepMaybeTraced, &step_counter,
    ...
    if (game.cart.find("Triangle") != string::npos &&
	step_counter > 2)
      exit(0);
  };
...
int main(int argc, char **argv) {
  ...
  // XXX
  MAKE_COMPREHENSIVE = true;
  SerialResult sr = RunGameToCollage(cobratriangle);
