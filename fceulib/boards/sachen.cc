...
static void S8259Synco(void) {
  ...
  if(!fceulib__.unif->UNIFchrrama) {
	...
	bank=(latch[x]&0x7)|((latch[4]&7)<<3);
      switch (type) {
      case 00: bank=(bank<<1)|(x&1); fceulib__.cart->setchr2(0x800*x,bank); break;
      case 01: fceulib__.cart->setchr2(0x800*x,bank); break;
      case 02: bank=(bank<<2)|(x&3); fceulib__.cart->setchr2(0x800*x,bank); break;
...
{
  fceulib__.cart->setprg32(0x8000,((latch[0]&0x80)>>6)|((latch[0]>>2)&1));
