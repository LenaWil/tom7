...
practical, but since the space is exponential ($256^{n}$), we need to
be smart here.
...
subtracted from the savestate prior to compression (if it's longer or
shorter, it's treated as an infinite repetition of those bytes). If
...
differ much, so I save a state from any arbitrary frame in the game
and then use that as the basis. This factors out any common parts
and makes the states much more compressible: The average size drops
to 1870.41 bytes.
...
would probably help too; zlip implements LZW which is only mediocre.
