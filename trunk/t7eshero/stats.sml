    ...
    let
      ...
      val medals =
        ...
        then
            ...
            [Hero.PerfectMatch, Hero.Snakes, Hero.Stoic,
             Hero.Plucky, Hero.Pokey,
             Hero.AuthenticStrummer, Hero.AuthenticHammer]
        else nil

      (* get old records for this song, if any. Then decide if we made new
         records. *)
      val { percent = oldpercent, misses = oldmisses, medals = oldmedals } =
        case List.find (fn (sid, _) =>
                        Setlist.eq(sid, songid)) (Profile.records profile) of
          NONE => { percent = 0, misses = total + 1, medals = nil }
        | SOME (_, r) => r

      (* old and new, avoiding duplicates *)
      val allmedals = List.filter
        (fn m' => not (List.exists (fn m => m = m') oldmedals)) medals @
        oldmedals

      (* save profile right away *)
      (* Prevent from crashing if there were no notes -- XXX hax *)
      val total = if total = 0 then 1 else total
      val () = Profile.setrecords profile
        ((songid, { percent = Int.max(oldpercent,
                                      (hit * 100) div total),
      ...
      val total_time =
        Real.fromInt (Word32.toInt (SDL.getticks() - start_time)) / 1000.0

      (* got a new medal? then get clothes. *)
      val award =
        if List.exists (fn m =>
                        not (List.exists (fn m' => m = m') oldmedals)) medals
