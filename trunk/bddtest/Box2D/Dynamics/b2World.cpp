...
void b2World::Solve(const b2TimeStep& step)
{
  ...
  for (b2Body* b = m_bodyList; b; b = b->m_next) {
	...
	for (b2Body* seed = m_bodyList; seed; seed = seed->m_next)
	{
		...
		b2Vec2 XXX_xf = seed->GetTransform().position;
		printf("Try seed @%.2f,%.2f\n", XXX_xf.x, XXX_xf.y);
		...
		while (stackCount > 0)
		{
			...
			for (b2ContactEdge* ce = b->m_contactList; ce; ce = ce->next)
			{
				...
				printf(" .. edge %s\n",
				       contact->IsTouching() ? "touching" : "not-touching");
...
// XXXX DEbugggg
static const char *GetFixtureName(b2Fixture *f) {
...
void b2World::Step(float32 dt, int32 velocityIterations, int32 positionIterations)
{
    ...
    for (b2Contact *c = GetContactList(); c != NULL; c = c->GetNext()) {
      const char *name1 = GetFixtureName(c->GetFixtureA());
      const char *name2 = GetFixtureName(c->GetFixtureB());
      printf("Post-collide %s-%s Contact! ", name1, name2);
      if (c->IsTouching()) {
	printf("touching ");
      }
      const b2Manifold *manifold = c->GetManifold();
