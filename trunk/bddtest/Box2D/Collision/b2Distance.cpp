	...
	case b2Shape::e_circle:
		{
			...
			printf("Proxy CIRCLE %s >%s\n",
			       vtos(circle->m_p).c_str(),
			       rtos(circle->m_radius).c_str());
		}
		break;
	...
	case b2Shape::e_polygon:
		{
			...
			printf("Proxy POLYGON ");
			for (int i = 0; i < polygon->m_vertexCount; i++) {
			  printf("%s ", vtos(polygon->m_vertices[i]).c_str());
			}
			printf("\n");
		}
		break;
					...
					const b2DistanceProxy* proxyB, const b2Transform& transformB)
	{
		...
		if (m_count == 0)
		{
		  printf("new simplex cache\n");
			...
			printf("transforma: %s\n"
			       "transformb: %s\n",
			       xftos(transformA).c_str(),
			       xftos(transformB).c_str());
