...
get around to it before too much code is written in XESP.
...
parseable semantic document in XML\wcite{Category:ML programming
language family} form.

The author doesn't think much of musicals,\z\wcite{Musical theatre} to
be perfectly honest, so those are basically a no-go. He also feels
that the metric system\wcite{Metric system} and time zones\wcite{Time
zone} are pretty overrated. Ketchup on eggs is gross, but not quite as
gross as foie gras\wcite{Foie gras}, which more or less has the word
``gross'' in its name so duh.\z\wcite{Duh}

\subsection{Another Paragraph}

We have described Wikiplia, the free programming language that anyone
can edit. Unlike other programming languages, it supports a variety of
freedoms (...) and is explicitly scalable to new freedoms. ...
