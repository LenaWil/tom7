...
# define CHECK_RAM_STEP(i) do {                                 \
  ...
  auto StepMaybeTraced = [&emu](uint8 b) {
    ...
    // in here!
    const bool match = cx == 5785174017646289441; // 5038258109338116820ULL;
    TRACEF(".. %llu", cx);
    TRACEA(&fceulib__.cart->VPage[4096 >> 10][4096], 9);
  ...
  auto SaveAndStep = [&StepMaybeTraced, &step_counter,
    ...
    TRACEF("Saved.");
    // Tracing save games can cause false positives. These are not
    // required to be byte-for-byte identical (like we can change the
    // order in which keys are written).
    // TRACEV(save);
    ...
    TRACEF("RAM %ull", emu->RamChecksum());
    TRACEF("IMG %ull", emu->ImageChecksum());
