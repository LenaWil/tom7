...
// Mapper 090 is simpliest mapper hardware and have not extended nametable
...
static void mira() {
  if ((tkcom[0] & 0x20 && is209) || is211) {
    ...
    {
      for (x = 0; x < 4; x++)
        fceulib__.cart->setntamem(
            fceulib__.cart->CHRptr[0] +
                (((names[x]) & fceulib__.cart->CHRmask1[0]) << 10),
            0, x);
    } else  // Name tables can be RAM or ROM.
    {
      for (x = 0; x < 4; x++) {
        if ((tkcom[1] & 0x80) == (names[x] & 0x80))  // RAM selected.
          fceulib__.cart->setntamem(
              fceulib__.ppu->NTARAM + ((names[x] & 0x1) << 10), 1, x);
        else
          fceulib__.cart->setntamem(
              fceulib__.cart->CHRptr[0] +
                  (((names[x]) & fceulib__.cart->CHRmask1[0]) << 10),
...
static void tekprom() {
  ...
  switch (tkcom[0] & 7) {
    ...
    case 01:
      ...
      fceulib__.cart->setprg16(0x8000,
    ...
    case 05:
      ...
      fceulib__.cart->setprg16(0x8000,
                               (prgb[1] & 0x1F) | ((tkcom[3] & 6) << 4));
      fceulib__.cart->setprg16(0xC000,
...
static void tekvrom() {
  ...
  switch (tkcom[0] & 0x18) {
    ...
    case 0x10:  // 2KB
      for (x = 0; x < 8; x += 2)
        fceulib__.cart->setchr2(
            x << 10, ((chrlow[x] | (chrhigh[x] << 8)) & mask) | bank);
      break;
    case 0x18:  // 1KB
      for (x = 0; x < 8; x++)
        fceulib__.cart->setchr1(
            x << 10, ((chrlow[x] | (chrhigh[x] << 8)) & mask) | bank);
      break;
  }
}
...
static DECLFW(M90IRQWrite) {
  ...
  switch (A & 7) {
    case 00:  // FCEU_printf("%s IRQ (C000)\n",V&1?"Enable":"Disable");
      IRQa = V & 1;
      if (!(V & 1)) fceulib__.X->IRQEnd(FCEU_IQEXT);
      break;
