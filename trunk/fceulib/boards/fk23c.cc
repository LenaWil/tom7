...
static void BMCFK23CPW(uint32 A, uint8 V) {
  ...
  // 1)|(fceulib__.unif->UNIFchrrama?((EXPREGS[2] & 0x40)<<3):0);
...
static DECLFW(BMCFK23CHiWrite) {
  if (EXPREGS[0] & 0x40) {
    ...
    if ((A == 0x8001) && (EXPREGS[3] & 2 && MMC3_cmd & 8)) {
      ...
      if (fceulib__.unif->UNIFchrrama) {  // hacky... strange behaviour, must be
                                          // bit scramble due to pcb layot
                                          // restrictions
...
static DECLFW(BMCFK23CWrite) {
  if (A & (1 << (dipswitch + 4))) {
    ...
    // however, the low 4 bits are known to control the mapping mode, so 0x20 is
    // more likely to be the immediate remap flag
  ...
  if (is_BMCFK23CA) {
    if (EXPREGS[3] & 2)
      EXPREGS[0] &= ~7;  // hacky hacky! if someone wants extra banking, then
