(* Retracts a board with some hugbots on it.
...
*)
functor Hugbot(val permute : bool
               val filename : string) :> 
sig 
    include GAME
(* val parse : string -> state *)
    val nbots : int
end =
struct
  ...
  (* XXX should test to make sure this is big
     enough to represent the board without overflow,
     and that we have the right number of words in
     our vector! *)

  (* turn number, 
     position vector of length nbots + 1, where 0 is the player *)
  (* type state = int * BW.word vector *) 
  fun state_turn v = BW.toInt (F.sub(v, 0))
  fun state_pos (v, n) = F.sub(v, n + 1)

  fun state_tabulate (turn : int, num : int, f : int -> BW.word) =
      F.tabulate (num + 1, fn 0 => BW.fromInt turn | n => f (n - 1))
...
*)
    ...
    let
      ...
      fun readuntil c =
        case TextIO.input1 f of
          NONE => raise Hugbot ("eof looking for chr " ^ 
                               Int.toString (ord c))
      ...
      fun uchar #"#" = WALL
        ...
        | uchar c = 
          ...
          else raise Hugbot ("Unexpected char: " ^ Char.toString c)
      ...
      fun parse i =
        (case TextIO.input1 f of
           NONE => raise Hugbot ("Unexpected EOF: require ^ at end")
         | SOME #"!" =>
             let 
                 val c = case TextIO.input1 f of
                              NONE => raise Hugbot "EOF after !"
                            | SOME c => c
             in
                 if ord c >= ord #"0"
                 andalso ord c <= ord #"9"
                 then t := (ord c - ord #"0")
                 else raise Hugbot "need turn number after !";
                 readuntil #"\n";
                 parse i
             end
         | SOME #"<" => 
      ...
      val startbots =
          Vector.tabulate(nbots, fn i => BW.fromInt (valOf (Array.sub(bots, i))))
    in
      ...
      { width = !w,
        ...
        initguy = 
        (case !g of
           NONE => raise Hugbot ("There is no guy (G)")
         | SOME i => BW.fromInt i),
        nbots = nbots,
        initbots = startbots,
        board = Vector.tabulate (!w * h,
                                 (fn i => Array.sub(arr, i))) }
    end handle (e as Hugbot s) => (print ("parse error: " ^ s ^ "\n");
                                  raise e)

  (* basically a hack since we can't override the one constant
     board. *)
  fun parse f =
      let
          val { nbots, initguy, initbots, ... } = parseboard f
      in
          (0, Vector.tabulate(nbots + 1,
                              fn 0 => initguy
                               | n => Vector.sub(initbots, n - 1)))
      end

  val { width, nbots, initturn, initguy, initbots, board } =
      parseboard filename

  val () = print ("There are " ^ Int.toString nbots ^ " bots.\n");
  ...
  val initials = 
      if permute 
      then
          (* every arrangement of bots in the final positions *)
          map (fn l => 
               ...
               in
                   ...
                   (initturn,
                    ...
                    fn 0 => initguy
                     | n => Vector.sub(v, n - 1))
               end) (ListUtil.permutations (Vector.foldr op:: nil initbots))
      else [state_tabulate (initturn, nbots + 1,
                            fn 0 => initguy
                             | n => Vector.sub(initbots, n - 1))]
  ...
  fun app_successors (f, state) =
      let
          ...
          fun isentat p =
              ...
              in 
                  (* XXX we pack the turn in here, so we
                     can't use exists directly. 
                  F.exists (fn p' => p = p') ents
                  *)
                  (* skip turn *)
                  ex 1
              end
                  ...
                  case travel pos dir of
                    ...
                    | SOME p' => 
                          ...
                          case Vector.sub(board, p') of
                            ...
                            | EXIT => isentat p'
                            | _ => isentat p' andalso blocked1 p'
              end
          ...
          (* then this function is the same for any kind of
            ...
            (* staying is easy.. *)
            fun go STAY _ = f (state_tabulate(moving,
                                              nbots + 1,
                                              fn i => state_pos(state, i)), 1)
              | go dir opp =
                    ...
                    in
                        ...
                        *)
                        case (Int.compare (hx, px),
                          ...
                          | (EQUAL, EQUAL) => raise Hugbot "impossible--on player!"
                    end
        end
