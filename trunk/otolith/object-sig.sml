...
sig
  ...
  val closestedge : object -> int * int -> node * node * int * int
  ...
  val splitedge : object -> int * int -> node option

  val triangles : object -> triangle list
  val nodes : object -> node list

  (* getnodewithin s (x, y) radius
     Get the closest node, as long as it is within the radius. *)
  val getnodewithin : object -> int * int -> int -> node option

(*
  val toworld : object -> WorldTF.object
  val fromworld : WorldTF.object -> object
*)

  (* Checks the structure of the object. This should never be
     necessary, but is useful if you suspect a bug. *)
 (*   val check : object -> unit *)
